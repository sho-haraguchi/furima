...
1. カリキュラムの「クレジットカード決済機能ミニアプリを作成しよう(PAY.JP API V2版)」の中盤に記載されている研修用アカウント登録URLからPAY.JPのアカウント登録を行う。
2. ログイン後、左サイドバーのAPI設定を開き、「テスト秘密鍵」と「テスト公開鍵」をコピー＆ペーストする。
3. 以下のコマンドを利用中のターミナルで実行し、環境変数として公開鍵と秘密鍵を設定する。
...
4. 購入画面で以下を入力（参考：https://docs.pay.jp/v1/testcard）  
